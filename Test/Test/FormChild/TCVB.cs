...
{
    public partial class TCVB : Form
    {

        private string maNVBH;
        public TCVB(string maNVBanHang)
        {
            InitializeComponent();
            maNVBH = maNVBanHang;
        }
        ...
        private void TCVB_Load(object sender, EventArgs e)
        {
            ...
            // Căn chỉnh table
            function.CanChinhDGV(dgvTCVB);
            dgvTCVB.Columns["Tên sản phẩm"].AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
        }
        ...
        private void loadDataSach()
        {
            string sqlSach = "Select " +
                ...
                "gia AS [Đơn giá]" +
                " from SanPham Where maTheLoai = 'TCVB'";

            dgvTCVB.DataSource = connectionSQL.hienDL(sqlSach);
        }
        ...
        private void txtTimKiem_TextChanged(object sender, EventArgs e)
        {
            if (cbbTimKiem.Text == "Tên sách")
            {
                string sqlSach = "Select " +
                    ...
                    "gia AS [Đơn giá]" +
                    "FROM SanPham WHERE maTheLoai = 'TCVB' and tenSanPham LIKE N'%" + txtTimKiem.Text.Trim() + "%'";

                dgvTCVB.DataSource = connectionSQL.hienDL(sqlSach);
                labelKQTimKiem.Text = $"Kết quả tìm thấy: {dgvTCVB.Rows.Count}";
            }
            if (cbbTimKiem.Text == "Mã sách")
            {
                string sqlSach = "Select " +
                    ...
                    "soLuongTon AS [Số lượng tồn]," +
                    "gia AS [Đơn giá]" +
                    "FROM SanPham WHERE maTheLoai = 'TCVB' and maSanPham LIKE N'%" + txtTimKiem.Text.Trim() + "%'";
        ...
        private void dgvTCVB_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            int i = dgvTCVB.CurrentRow.Index;

            txtMaSach.Text = dgvTCVB.Rows[i].Cells[0].Value?.ToString() ?? "";
            txtTenSach.Text = dgvTCVB.Rows[i].Cells[1].Value?.ToString() ?? "";

            if (dgvTCVB.Rows[i].Cells[3].Value != null)
            {
                decimal donGia;
                if (decimal.TryParse(dgvTCVB.Rows[i].Cells[3].Value.ToString(), out donGia))
                {
        ...
        private void dgvTCVB_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {

    ...
    public partial class Truyen : Form
    {
        ...
        private void loadDataTruyen()
        {
            string sqlSach = "Select " +
                ...
                "gia AS [Đơn giá]" +
                " from SanPham Where maTheLoai = 'Truyen'";

            dgvTruyen.DataSource = connectionSQL.hienDL(sqlSach);
        }
        ...
        private void dgvTruyen_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            int i = dgvTruyen.CurrentRow.Index;

            txtMaTruyen.Text = dgvTruyen.Rows[i].Cells[0].Value?.ToString() ?? "";
            txtTenTruyen.Text = dgvTruyen.Rows[i].Cells[1].Value?.ToString() ?? "";

            if (dgvTruyen.Rows[i].Cells[3].Value != null)
            {
                decimal donGia;
                if (decimal.TryParse(dgvTruyen.Rows[i].Cells[3].Value.ToString(), out donGia))
                {
        ...
        private void txtTimKiem_TextChanged(object sender, EventArgs e)
        {
            if (cbbTimKiem.Text == "Tên truyện")
            {
                ...
                dgvTruyen.DataSource = connectionSQL.hienDL(sqlSach);
                labelKQTimKiem.Text = $"Kết quả tìm thấy: {dgvTruyen.Rows.Count}";
            }
            if (cbbTimKiem.Text == "Mã truyện")
            {
                string sqlSach = "Select " +
                "maSanPham AS [Mã sản phẩm]," +
                "tenSanPham AS [Tên sản phẩm]," +
        ...
        private void dgvTruyen_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlCommand.Parameters.AddWithValue("maSP", txtMaTruyen.Text);
                sqlCommand.Parameters.AddWithValue("tenSP", txtTenTruyen.Text);
        ...
        private void btnThemVaoGioHang_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(txtMaTruyen.Text))
                {
                    MessageBox.Show("Vui lòng chọn sản phẩm!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                string maSP = txtMaTruyen.Text;
                string tenSP = txtTenTruyen.Text;
